// lib/screens/driver_session_screen.dart
import 'dart:async';
import 'package:flutter/material.dart';
import '../auth/api_service.dart';
import '../auth/session_manager.dart';
import '../screens/driver_ride_tracking_screen.dart';

class DriverSessionsScreen extends StatefulWidget {
  const DriverSessionsScreen({super.key});
  @override
...
class _DriverSessionsScreenState extends State<DriverSessionsScreen>
  ...
  final _scrollController = ScrollController(); // ← 1. ADDED
  ...
  void dispose() {
    ...
    _scrollController.dispose(); // ← 2. ADDED
    super.dispose();
  }
...
Future<void> _loadActive(int id) async {
    ...
    try {
      final d = await ApiService.getDriverActiveTrips(id);
      if (mounted) {
  ...
  Future<void> _loadCompleted(int id) async {
    ...
    try {
      final d = await ApiService.getCompletedDriverTrips(id);
      if (mounted) setState(() { _completed = d ?? []; _loadingC = false; });
  ...
  Future<void> _updateStatus(int bookingId, String status) async {
    final offset = _scrollController.offset; // ← 3. ADDED: save position
    try {
      await ApiService.updateBookingStatus(bookingId, status);
      _snack(_statusMsg(status), _statusColor(status));
      final id = await SessionManager.getUserId();
      if (id != null) {
        await _loadActive(id);
        if (status == 'completed') await _loadCompleted(id);
      }
      WidgetsBinding.instance.addPostFrameCallback((_) { // ← 3. ADDED: restore position
  ...
  String _statusMsg(String s) {
    switch (s) {
      ...
      case 'in_trip':   return 'Trip started!';
  ...
  String _name(Map r) {
    ...
    return n.isNotEmpty ? n : 'Passenger';
  }
  ...
  Map<String, dynamic> _statusStyle(String s) {
    switch (s) {
      ...
      case 'in_trip':  return {'bg': const Color(0xFFEEF2FF), 'text': kAccent,                 'label': 'In Trip'};
 ...
 DateTime? _parseScheduled(Map r) {
  ...
  print('PARSE DEBUG: date="$dateStr" time="$timeStr" keys=${r.keys.toList()}');
  if (dateStr.isEmpty || timeStr.isEmpty) return null;
  try {
    final result = DateTime.parse('${dateStr.substring(0, 10)} $timeStr');
  ...
  String _countdownText(Map r) {
    ...
    if (diff.inDays > 0) return 'Trip starts in ${diff.inDays} day${diff.inDays > 1 ? "s" : ""} (${_fmtDate(r["date"]?.toString())} at ${_fmtTime(r["service_time"]?.toString() ?? r["booking_time"]?.toString())})';
    if (diff.inHours > 0) return 'Trip starts in ${diff.inHours}h ${diff.inMinutes % 60}m';
    return 'Trip starts in ${diff.inMinutes} minute${diff.inMinutes != 1 ? "s" : ""}';
  }
      ...
      child: SafeArea(child: Column(children: [
        Padding(
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.center, children: [
            const Text('Driver Trips', style: TextStyle(fontSize: 26, fontWeight: FontWeight.w900, color: kDark)),
            const SizedBox(height: 4),
            const Text('Manage your ride sessions', style: TextStyle(fontSize: 13, color: kMuted, fontWeight: FontWeight.w600)),
          ]),
        ),
        Container(
        ...
        Expanded(child: TabBarView(controller: _tab, children: [
          _loadingA
              ...
              : _active.isEmpty
                  ? _emptyState('No active trips', 'Accepted trips appear here', Icons.directions_car_rounded)
                  : RefreshIndicator(onRefresh: _refreshAll, color: kPrimary,
                      child: ListView.builder(
                        controller: _scrollController, // ← 4. ADDED
          ...
          _loadingC
              ? const Center(child: CircularProgressIndicator(color: kPrimary))
              : _completed.isEmpty
                  ? _emptyState('No completed trips', 'Finished trips appear here', Icons.check_circle_outline_rounded)
                  : RefreshIndicator(onRefresh: _refreshAll, color: kPrimary,
                      child: ListView.builder(
                        padding: const EdgeInsets.fromLTRB(20, 0, 20, 30),
  ...
  Widget _activeCard(Map r) {
    ...
    // Check if ANY trip is currently in_trip
    final bool anyInTrip = _active.any(
      (t) => (t['status'] ?? '').toString().toLowerCase() == 'in_trip'
    );
    // This card is the active one
    final bool isThisInTrip = status == 'in_trip';
    // Lock other cards if something else is in_trip
    final bool locked = anyInTrip && !isThisInTrip;
    ...
    return Opacity(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          ...
          // Highlight the in-trip card with a green border
          border: isThisInTrip
        ...
        child: Column(children: [
          // "IN PROGRESS" banner at the top for active trip
          if (isThisInTrip)
            Container(
          ...
          Container(
            ...
            decoration: BoxDecoration(
              ...
              borderRadius: isThisInTrip
                  ? BorderRadius.zero
                  : const BorderRadius.vertical(top: Radius.circular(20)),
            ),
            child: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
              Text('Booking #${r["booking_id"]}',
                  style: TextStyle(fontSize: 12, fontWeight: FontWeight.w800, color: sd['text'] as Color)),
              Row(children: [
                _scheduleBadge(r),
                const SizedBox(width: 6),
                Container(
                  padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 4),
                  decoration: BoxDecoration(
                    color: (sd['text'] as Color).withOpacity(0.12),
                    borderRadius: BorderRadius.circular(999),
                    border: Border.all(color: (sd['text'] as Color).withOpacity(0.25)),
                  ),
                  child: Text(sd['label'] as String,
                      style: TextStyle(fontSize: 11, fontWeight: FontWeight.w900, color: sd['text'] as Color)),
                ),
              ]),
            ]),
          ),
            ...
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Row(children: [
                Container(
                  width: 44, height: 44,
                  decoration: BoxDecoration(
                    gradient: const LinearGradient(colors: [Color(0xFF4B4F83), Color(0xFF6470D2)]),
                    borderRadius: BorderRadius.circular(12),
                  ),
                ...
                Container(
                  ...
                  child: Text('${net.toStringAsFixed(2)} EGP',
                      style: const TextStyle(fontSize: 13, fontWeight: FontWeight.w900, color: kGreen)),
                ),
              ]),
              const SizedBox(height: 12),
              const Divider(height: 1, color: Color(0xFFE8EBF5)),
              const SizedBox(height: 12),
              _row(Icons.my_location_rounded,   'Pickup: ${r["address"] ?? "—"}'),
              _row(Icons.location_on_rounded,    'Destination: ${r["destination"] ?? "—"}'),
              _row(Icons.calendar_today_rounded, _fmtDate(r['date']?.toString())),
              _row(Icons.access_time_rounded,    _fmtTime((r['service_time'] ?? r['booking_time'])?.toString())),
              const SizedBox(height: 14),

              // If locked by another in-trip booking, show a message instead of buttons
              if (locked)
                Container(
                  width: double.infinity,
                  ...
                  child: const Row(children: [
                    ...
                    SizedBox(width: 10),
                    Expanded(child: Text('Complete your current trip first',
                ...
                if (!actionable) ...[
                  ...
                  _btn('Map & Track', kPrimary, () async {
                    await Navigator.push(context, MaterialPageRoute(
                      builder: (_) => DriverRideTrackingScreen(booking: r),
                    ));
                    // Auto refresh when driver comes back from tracking screen
                    final id = await SessionManager.getUserId();
                    if (id != null) await Future.wait([_loadActive(id), _loadCompleted(id)]);
  ...
  Widget _completedCard(Map r) {
    final gross    = double.tryParse(r['payment_total']?.toString() ?? '0') ?? 0;
    final net      = gross * 0.85;
    final rating = r['driver_patient_rating']?.toString();
    final hasRating = rating != null && rating.isNotEmpty && rating != 'null';
  ...
  Row(mainAxisAlignment: MainAxisAlignment.center,
    children: List.generate(5, (i) => GestureDetector(
      onTap: () async {
        ...
        final ok = await ApiService.ratePatient(
          bookingId: int.parse(r['booking_id'].toString()),
          driverId: driverId,
          rating: i + 1,
        );
        _snack(
          ok ? 'Passenger rated!' : 'Error rating passenger',
          ok ? kGreen : Colors.red,
        );
        if (ok) {
          final id = await SessionManager.getUserId();
          if (id != null) await _loadCompleted(id);
        }
      },
      child: Padding(
        padding: const EdgeInsets.symmetric(horizontal: 4),
        child: const Icon(Icons.star_outline_rounded,
            color: Color(0xffD5A72C), size: 36),
      ),
    ))),
  const SizedBox(height: 4),
],
if (hasRating) ...[
  ...
  const SizedBox(height: 10),
  Row(children: [
    const Icon(Icons.star_rounded, color: Color(0xffD5A72C), size: 18),
    const SizedBox(width: 4),
    Text('You rated passenger: $rating / 5',
        style: const TextStyle(fontSize: 13,
            fontWeight: FontWeight.w800, color: Color(0xff8A6700))),
    const Spacer(),
    Text('Booking #${r["booking_id"]}',
  ...
  void _confirmComplete(int bookingId) {
    showDialog(context: context, builder: (_) => AlertDialog(
      ...
      title: const Text('Complete Trip?', style: TextStyle(fontWeight: FontWeight.w900, color: kDark)),
      content: const Text('This will mark the trip as completed and add earnings to your wallet.',
