...
class _SessionsScreenState extends State<SessionsScreen>
  ...
  Future<void> _loadActive(int id) async {
    ...
    try {
      final d = await ApiService.getActiveSessions(id);
      if (mounted) {
  ...
  Future<void> _loadCompleted(int id) async {
    ...
    try {
      final d = await ApiService.getCompletedSessions(id);
  ...
  String _statusMsg(String s) {
    switch(s) {
      ...
      case 'in_trip':   return 'Care session started!';
  ...
  Map<String, dynamic> _statusStyle(String s) {
    switch(s) {
      ...
      case 'in_trip':  return {'bg': const Color(0xFFEEF2FF), 'text': kAccent,                 'label': 'In Care'};
      ...
      child: SafeArea(child: Column(children: [
        Padding(
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.center, children: [
            ...
            const Text('Manage your care sessions', style: TextStyle(fontSize: 13, color: kMuted, fontWeight: FontWeight.w600)),
          ]),
        ),
        Container(
        ...
        Expanded(child: TabBarView(controller: _tab, children: [
          _loadingA
              ...
              : _active.isEmpty
                  ? _emptyState('No active sessions', 'Accepted sessions appear here', Icons.calendar_today_rounded)
          ...
          _loadingC
              ? const Center(child: CircularProgressIndicator(color: kPrimary))
              : _completed.isEmpty
                  ? _emptyState('No completed sessions', 'Finished sessions appear here', Icons.check_circle_outline_rounded)
                  : RefreshIndicator(onRefresh: _refreshAll, color: kPrimary,
                      child: ListView.builder(
                        padding: const EdgeInsets.fromLTRB(20, 0, 20, 30),
          ...
          child: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
            ...
            Row(children: [
              ...
              Container(
                padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 4),
                decoration: BoxDecoration(
                  color: (sd['text'] as Color).withOpacity(0.12), borderRadius: BorderRadius.circular(999),
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            if (_isActionable(r)) ...[
              ...
              if (status == 'accepted' || status == 'arrived') ...[
                ...
                _btn('Start Care', kAccent, () => _updateStatus(bid, 'in_trip'), icon: Icons.play_arrow_rounded),
              ],
              if (status == 'in_trip')
                _btn('Complete Session', kGreen, () => _confirmComplete(bid), icon: Icons.check_circle_rounded),
            ],
          ]),
        ),
      ]),
  ...
  void _confirmComplete(int bookingId) {
    showDialog(context: context, builder: (_) => AlertDialog(
      ...
      title: const Text('Complete Session?', style: TextStyle(fontWeight: FontWeight.w900, color: kDark)),
      content: const Text('This will mark the session as completed and add earnings to your wallet.',
